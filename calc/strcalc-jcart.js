...
var productname = ""; //Clear product name
var crosstext = ""; //Clear crossbar product text
var temptext = ""; //Clear temporary crossbar text


//Get type of stretcher
var style = parseFloat(document.forms.strpricer.strstyle.value, 10);
var stindex = document.forms.strpricer.strstyle.selectedIndex;
var stylename = document.forms.strpricer.strstyle.options[stindex].text;
...
//Begin describing product
productname = strlength + '\"x' + strheight + '\" ' + stylename;

//Calculate number of braces
...
//Check to see if customer has decided to use her own number
var override = document.forms.strpricer.override.checked;

if (override){
	crosstext = " (Customer set #.)";
...
if (!(override)){
	...
	lrecomend = "our min. recommendation of ";
	hrecomend = "our min. recommendation of ";
	if ((blength && (blength != lengthcross)) || (bheight && (bheight != heightcross)))
	{strtext += "Crossbar numbers have been changed to reflect our recommendations.  To change these numbers, check the Override box and recalculate.\n"; }
	} else {
	lengthcross = blength;
  heightcross = bheight;
       }
...
if (lengthcross){
   temptext += "Price includes " + lrecomend + lengthcross + " " + strheight + "\" " + lcrosstext + " lengthwise";
   linearft = linearft + (lengthcross * strheight)/12;
   if (heightcross) {
   	temptext += " and " + hrecomend + heightcross + " " + strlength + "\" " + hcrosstext + " heightwise.  ";
   	linearft = linearft + (heightcross * strlength)/12;
    }else {temptext = temptext + ".  "}
}else if (heightcross) {
	temptext += "Price includes " + hrecomend + heightcross + " " + strlength + "\" " + hcrosstext + " heightwise.  ";
    	linearft = linearft + (heightcross * strlength)/12;
    	}

crosstext = temptext + crosstext;
strtext += temptext;

productname = productname + ". " + crosstext;
...
if (collapse){
  ...
  }else {collapseprice = .40; productname += " Collapsible.";}
}
...
if (folds){
  ...
  if (lengthcross){
        ...
        productname += " Includes " + folds  + " folds through " +lengthcross + " crossbars.";
                  }                
